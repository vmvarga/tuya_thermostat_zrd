 ...
 */
_attribute_ram_code_sec_ static inline int app_flash_is_busy(void){
 ...
 * @return		none.
 */
_attribute_ram_code_sec_noinline_ static void app_flash_send_cmd(unsigned char cmd){
	mspi_high();
	sleep_us(1);
	mspi_low();
	mspi_write(cmd);
 ...
 * @return		none.
 */
_attribute_ram_code_sec_noinline_ static void app_flash_send_addr(unsigned int addr){
 ...
 * @return    none.
 */
_attribute_ram_code_sec_noinline_ static void app_flash_wait_done(void)
{
	sleep_us(100);
	app_flash_send_cmd(FLASH_READ_STATUS_CMD_LOWBYTE);

	int i;
	for(i = 0; i < 10000000; ++i){
		if(!app_flash_is_busy()){
			break;
		}
	}
	mspi_high();
 ...
 * @return 		none.
 */
_attribute_ram_code_sec_noinline_ void app_flash_mspi_read_ram(unsigned char cmd, unsigned long addr, unsigned char addr_en, unsigned char dummy_cnt, unsigned char *data, unsigned long data_len)
{
	unsigned char r = irq_disable();

	app_flash_send_cmd(cmd);
	if(addr_en)
	{
	    app_flash_send_addr(addr);
	}
 ...
 */
_attribute_ram_code_sec_noinline_ unsigned char app_flash_mspi_write_ram(unsigned char cmd, unsigned long addr, unsigned char addr_en, unsigned char *data, unsigned long data_len)
{
	unsigned char r = irq_disable();
	unsigned char ret = 1;

	app_flash_send_cmd(FLASH_WRITE_ENABLE_CMD);
	app_flash_send_cmd(cmd);
	if(addr_en)
	{
		if(g_flashIsZbit){
 ...
 */
_attribute_ram_code_sec_ void app_flash_erase_sector(unsigned long addr)
{
    app_flash_mspi_write_ram(FLASH_SECT_ERASE_CMD, addr, 1, NULL, 0);
}
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ void app_flash_read_page(unsigned long addr, unsigned long len, unsigned char *buf)
{
    app_flash_mspi_read_ram(FLASH_READ_CMD, addr, 1, 0, buf, len);
}
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
_attribute_ram_code_sec_ void app_flash_write_page(unsigned long addr, unsigned long len, unsigned char *buf)
{
	...
	do{
		nw = len > ns ? ns :len;
		if(app_flash_mspi_write_ram(FLASH_WRITE_CMD, addr, 1, buf, nw) == 0){
			break;
		}
		ns = PAGE_SIZE;
		addr += nw;
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
unsigned char app_flash_read_status(unsigned char cmd)
{
	unsigned char status = 0;
	app_flash_mspi_read_ram(cmd, 0, 0, 0, &status, 1);
	return status;
}
 ...
 */
//__attribute__((weak))
_attribute_ram_code_sec_ void app_flash_write_status(flash_status_typedef_e type , unsigned short data)
{
	unsigned char buf[2];
	...
	if(type == FLASH_TYPE_8BIT_STATUS){
	    app_flash_mspi_write_ram(FLASH_WRITE_STATUS_CMD_LOWBYTE, 0, 0, buf, 1);
	}else if(type == FLASH_TYPE_16BIT_STATUS_ONE_CMD){
	    app_flash_mspi_write_ram(FLASH_WRITE_STATUS_CMD_LOWBYTE, 0, 0, buf, 2);
	}
}
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
unsigned int app_flash_read_raw_mid(void)
{
	unsigned int flash_mid = 0;
	app_flash_mspi_read_ram(FLASH_GET_JEDEC_ID, 0, 0, 0, (unsigned char*)(&flash_mid), 3);
	return flash_mid;
}
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
unsigned int app_flash_read_mid(void)
{
	unsigned int flash_mid = 0;
	app_flash_mspi_read_ram(FLASH_GET_JEDEC_ID, 0, 0, 0, (unsigned char*)(&flash_mid), 3);
	...
	if(flash_mid == 0x1460c8)
	{
		...
		app_flash_mspi_read_ram(0x5a, 0x00, 1, 1, buf, 4);
 ...
 *              If an abnormality occurs, the firmware and user data may be rewritten, resulting in the final Product failure)
 */
void app_flash_read_uid(unsigned char idcmd, unsigned char *buf)
{
	if(idcmd == FLASH_READ_UID_CMD_GD_PUYA_ZB_TH)	//< GD/PUYA/ZB/TH
	{
	    app_flash_mspi_read_ram(idcmd, 0x00, 1, 1, buf, 16);
	}
}
 ...
 */
unsigned int app_flash_get_vendor(unsigned int flash_mid)
{
 ...
 */
unsigned char app_flash_is_zb(void)
{
 ...
 * @return		none.
 */
void app_flash_safe_voltage_set(unsigned short vol)
{
	g_flashWorkVol = vol;
}
 ...
 * @return		none.
 */
void app_flash_vdd_f_calib(void)
{
